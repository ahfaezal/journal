...
from app.api.journal import (
    read_full_paper,
    read_generated_sections,
    read_section_structure,
)
...
def get_reviewer_report_json_path(project_id: str, paper_id: str) -> Path:
    output_dir = GENERATED_OUTPUT_ROOT / project_id
    output_dir.mkdir(parents=True, exist_ok=True)
    return output_dir / f"reviewer_report_{paper_id}.json"


def get_reviewer_report_markdown_path(project_id: str, paper_id: str) -> Path:
    output_dir = GENERATED_OUTPUT_ROOT / project_id
    output_dir.mkdir(parents=True, exist_ok=True)
    return output_dir / f"reviewer_report_{paper_id}.md"


def read_reviewer_report(project_id: str, paper_id: str) -> dict[str, Any] | None:
    output_path = get_reviewer_report_json_path(project_id, paper_id)
...
@router.post("/{project_id}/{paper_id}/run")
def run_project_reviewer_simulation(project_id: str, paper_id: str) -> dict[str, Any]:
    paper_id = paper_id.upper()
    report = build_reviewer_report_from_outputs(project_id, paper_id)
    json_path = get_reviewer_report_json_path(project_id, paper_id)
    markdown_path = get_reviewer_report_markdown_path(project_id, paper_id)

    report = safe_write_json(json_path, report, status="reviewed")
...
def get_project_reviewer_simulation(project_id: str, paper_id: str) -> dict[str, Any]:
    paper_id = paper_id.upper()
    report = read_reviewer_report(project_id, paper_id)
    if report:
        return report

    return build_reviewer_report_from_outputs(project_id, paper_id)


def build_reviewer_report_from_outputs(project_id: str, paper_id: str) -> dict[str, Any]:
